...
public class ResendEmailProvider implements EmailProvider {
    ...
    public ResendEmailProvider(@Value("${notification.email.resend.api-key}") String apiKey,
    ...
    public String send(EmailMessage message) {
        String json = "{\"from\":\"" + escape(message.from()) + "\",\"to\":[\"" + escape(message.to())
                + "\"],\"subject\":\"" + escape(message.subject()) + "\",\"text\":\"" + escape(message.text()) + "\"}";
        Request request = new Request.Builder().url(endpoint)
                .addHeader("Authorization", "Bearer " + apiKey)
                .post(RequestBody.create(json, JSON)).build();
        try (Response response = client.newCall(request).execute()) {
            if (!response.isSuccessful()) throw new IllegalStateException("Resend returned HTTP " + response.code());

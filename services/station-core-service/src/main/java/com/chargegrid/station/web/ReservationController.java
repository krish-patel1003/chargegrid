...
public class ReservationController {
    ...
    public ReservationController(StationService service) {
        this.service = service;
    }

    @PostMapping
    public Dtos.ReservationView reserve(
            @Valid @RequestBody Dtos.ReservationRequest request,
            @RequestHeader(value = "X-User-Id", defaultValue = "demo-driver") String owner) {
        return service.reserve(request.connectorId(), owner);
    }
    ...
    public Dtos.ReservationView get(
            ...
            @RequestHeader(value = "X-User-Id", defaultValue = "demo-driver") String owner) {
        return service.reservation(id, owner);
    }

    @PostMapping("/{id}/verify-start")
    public Dtos.SessionView start(
            @PathVariable String id,
            @Valid @RequestBody Dtos.CodeRequest request,
            @RequestHeader(value = "X-User-Id", defaultValue = "demo-driver") String owner) {
        return service.verifyStart(id, owner, request.code());
    }
}
